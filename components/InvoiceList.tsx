...
interface InvoiceListProps {
  ...
  onBulkMarkAsOpen: (invoiceIds: string[]) => void;
  onAddNew: () => void;
...
export const InvoiceList: React.FC<InvoiceListProps> = ({ invoices, userProfile, onView, onEdit, onDelete, onMarkAsPaid, onMarkAsOpen, onBulkDelete, onBulkMarkAsPaid, onBulkMarkAsOpen, onAddNew, isFreePlanLimitReached, setCurrentView }) => {
  ...
  const handleBulkMarkOpen = () => {
    onBulkMarkAsOpen(selectedInvoices);
    setSelectedInvoices([]);
  };
        ...
        {selectedInvoices.length > 0 && (
            <div className="bg-sky-100 border-l-4 border-sky-500 rounded-r-lg p-3 mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-sm">
                ...
                <div className="flex items-center space-x-2 flex-wrap gap-y-2">
                    ...
                    <button
                        onClick={handleBulkMarkOpen}
                        className="px-3 py-1.5 text-sm font-medium rounded-md bg-yellow-600 text-white hover:bg-yellow-700 transition-colors"
                    >
                        Markeer als Open
                    </button>
                    <button
                  ...
                  <tr key={invoice.id} className={`border-b border-stone-100 hover:bg-stone-100 cursor-pointer ${selectedInvoices.includes(invoice.id) ? 'bg-blue-50 hover:bg-blue-100' : ''}`} onClick={() => onView(invoice.id)}>
                    ...
                    <td className="p-3 text-right whitespace-nowrap" onClick={e => e.stopPropagation()}>
                      ...
                      {invoice.status === 'betaald' && (
                          <button onClick={() => onMarkAsOpen(invoice.id)} className="text-sm font-medium text-yellow-600 hover:text-yellow-800 mr-4">Markeer als Open</button>
                      )}

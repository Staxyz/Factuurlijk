  ...
  useEffect(() => {
    const processCheckout = async () => {
      try {
        ...
        // This handles Payment Link returns where Mollie doesn't provide a payment_id in the URL
        if (!paymentId && (isPaymentLinkReturn || isRecentPayment) && (storedUserId === currentUser.id || !storedUserId)) {
          console.log('📝 Payment Link return detected (no payment_id in URL), upgrading directly...');
          console.log('   Payment source:', paymentSource);
          console.log('   Stored user ID:', storedUserId);
          console.log('   Current user ID:', currentUser.id);
          console.log('   Is recent payment:', isRecentPayment);
          ...
          // Since user returned from Mollie payment and we have recent payment info,
          // upgrade directly - the payment was completed (user wouldn't be redirected otherwise)
          console.log('💳 User returned from Mollie payment, upgrading to Pro directly...');
          
          try {
            await upgradeUserDirectly(currentUser, paymentSource || 'upgrade-page');
            return;
          } catch (upgradeError) {
            console.error('❌ Direct upgrade failed:', upgradeError);
